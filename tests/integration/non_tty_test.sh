#!/bin/bash
# Non-TTY specific integration tests for AUSH shell
...
echo "======================================"
echo "AUSH Non-TTY Integration Tests"
echo "======================================"
echo ""
...
OUTPUT=$(echo -e "$COMMANDS" | "$AUSH_BINARY" 2>&1)
...
echo "echo success" | "$AUSH_BINARY" >/dev/null 2>&1
EXIT_CODE=$?
[ $EXIT_CODE -eq 0 ]
assert_success
...
OUTPUT=$(echo "echo first && echo second" | "$AUSH_BINARY" 2>&1)
echo "$OUTPUT" | grep -q "first" && echo "$OUTPUT" | grep -q "second"
assert_success

# Test 16: Simulate cron job scenario
test_case "Cron job simulation"
CRON_SCRIPT="/tmp/aush_cron_$$.sh"
...
CRONEOF
OUTPUT=$("$AUSH_BINARY" < "$CRON_SCRIPT" 2>&1)
rm -f "$CRON_SCRIPT"
...
CI_SCRIPT="/tmp/aush_ci_$$.sh"
cat > "$CI_SCRIPT" <<'CIEOF'
# CI/CD Test Script
echo "Running tests..."
echo "All tests passed"
CIEOF
OUTPUT=$("$AUSH_BINARY" < "$CI_SCRIPT" 2>&1)
rm -f "$CI_SCRIPT"
...
OUTPUT=$(echo "echo before EOF" | "$AUSH_BINARY" 2>&1)
EXIT_CODE=$?
