#!/bin/bash
# Pipeline integration tests for AUSH shell
...
echo -e "1\n2\n3\n4\n5" > "$TEMP_FILE"
OUTPUT=$("$AUSH_BINARY" -c "cat $TEMP_FILE | head -n 2" 2>&1)
...
echo -e "1\n2\n3\n4\n5" > "$TEMP_FILE"
OUTPUT=$("$AUSH_BINARY" -c "cat $TEMP_FILE | tail -n 2" 2>&1)
echo "$OUTPUT" | grep -q "5"
assert_success

# Test 11: Pipeline with sort
test_case "Pipeline: echo | sort"
OUTPUT=$("$AUSH_BINARY" -c "echo -e 'c\nb\na' | sort" 2>&1)
...
"$AUSH_BINARY" -c "echo test | cat" >/dev/null 2>&1
EXIT_CODE=$?
[ $EXIT_CODE -eq 0 ]
assert_success
...
"$AUSH_BINARY" -c "echo redirect | cat > $TEMP_FILE" 2>&1
...
OUTPUT=$("$AUSH_BINARY" -c "echo first | cat && echo second" 2>&1)
